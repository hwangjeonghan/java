...
public class FinalFieldtest {
    ...
    * 1. 지역변수 : 처기화 이후 값 변경 불가
    * 2. 매개변수 : 호출시 전달한 인자 변경불가
    * 3. 전역변수 : 인스턴스 생성 후 초기화 이후에 값 변경 불가
    * 4. 클래스(static) 변수 : 프로그램 start 이후 값 변경 불가능
    * 5. non-static 메소드 : 메소드 재장성(overriding) 불가
    * 6. static 메소드 : 메소드 재작성(overriding) 불사
    * 7. 클래스 : 상속 불가능
    * */
     ...
     * 하지만 static은 프로그램이 start 될 때 할 당되기 때문에 초기화가 되지 않는 상태로 선언된 것고 동일하여
...
//        STATIC_DOUBLE = staticDouble;
    ...
    * */
    ...
    // 그러나 아래와 같이 생성하게되면 초기값이 전달이 안되는 것이고
