...
public class ExceptionTest {
    ...
    public void checkEnoughMoney(int price, int money) throws PriceNegativeException, MonenyNegativeException, NotEnoughMoneyException {
        if (price < 0) {
            throw new PriceNegativeException("상품 가격은 음수 일 수 없습니다");
        }
        if (money < 0) {
            throw new MonenyNegativeException("가지고 있는 돈은 음수일 수 없습니다");
        }
        if (money < price) {
